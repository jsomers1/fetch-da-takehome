...
with base_set as (
	...
	left join products b on a.barcode=b.barcode
	left join users c on a.user_id=c.id),
-- get counts of metrics by each account age cohort
base_set2 as (
...
-- finally, compute the percent change in each metric year by year
base_set3 as (
select a.*, 
...
from base_set2 a)
-- lastly take the avg of those percent changes
